    ...
    def _onchange_dates(self):
        ...
        for product in self.env['product.product'].browse(product_ids):
            for _range in ranges:
                estimate = self.env['stock.demand.estimate'].search([
                    ('product_id', '=', product.id),
                    ('date_range_id', '=', _range.id),
                    ('location_id', '=', self.location_id.id)
                ], limit=1)
                if estimate:
                    uom_id = estimate.product_uom.id
                    uom_qty = estimate.product_uom_qty
...
class DemandEstimateWizard(models.TransientModel):
    ...
    def create_sheet(self):
        ...
        sheet = self.env['stock.demand.estimate.sheet'].with_context(
