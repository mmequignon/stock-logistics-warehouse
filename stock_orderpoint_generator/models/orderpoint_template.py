...
class OrderpointTemplate(models.Model):
    ...
    procurement_ids = fields.One2many(
        'procurement.order', 'orderpoint_id', copy=False
    )

    group_id = fields.Many2one('procurement.group', copy=True)
    ...
    product_uom = fields.Many2one(related='product_id.uom_id', required=False)
