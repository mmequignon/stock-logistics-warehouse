...
class StockMoveLineSerialGenerator(models.TransientModel):
    ...
    def _check_serials_range(self, new_serials):
        if self.last_code:
            if new_serials[-1] != self.last_code:
                raise ValidationError(
                    _(
                        "Last serial number doesn't belonging to the given range"
                    )
                )
    ...
    def _check_qty_to_process(self):
        ...
        if self.qty_to_process > max_qty:
            raise ValidationError(
                _(
                    "The qty received cannot exceed the number of move lines not "
                    "having a serial number yet. (%s)"
                )
                % max_qty
            )
    ...
    def get_sequence(self):
        ...
        return self.env['ir.sequence.wizard'].create(
            {'number_next_actual': self.stock_move_id.first_code}
        )
